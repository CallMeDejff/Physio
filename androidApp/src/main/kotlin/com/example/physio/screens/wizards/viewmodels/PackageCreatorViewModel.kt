...
class PackageCreatorViewModel @Inject constructor(
    ...
    fun loadExercises() {
        loadExercisesList(_exercisesList = _exercisesList, listService = listService,tag = PACKAGE_VIEWMODEL_TAG)
    }

    fun loadCondition() {
        loadConditionList(_conditionsList = _conditionsList, listService = listService, tag = PACKAGE_VIEWMODEL_TAG)
    }
    ...
    fun onAssignPackageClick(navigate: (String) -> Unit) {
        launchCatching(
            ...
            block = {
                ...
                exercisePackageService.assignPackageToUser(userId = _selectedUsers.value.first(), packageId = _selectedPackages.value.first())
                navigate(WizardScreen.CreatorWizard.route)
