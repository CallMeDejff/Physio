...
class StorageSampleImpl @Inject constructor(
    ...
    override suspend fun setSampleData() {
        ...
        val bodyPartsData = mapOf(
            "body_parts" to listOf(
                mapOf("id" to "body_part_1", "name" to "Chest"),
                mapOf("id" to "body_part_2", "name" to "Back"),
                mapOf("id" to "body_part_3", "name" to "Legs"),
                mapOf("id" to "body_part_4", "name" to "Arms")
            )
        )
        ...
        try {
            ...
            val bodyPartsDocument =
                firestore.collection(SUMMARY_COLLECTION).document("body_parts").get().await()
            if (!bodyPartsDocument.exists()) {
                firestore.collection(SUMMARY_COLLECTION)
                    .document("body_parts")
                    .set(bodyPartsData)
                    .await()
                Log.d(STORAGE_SERVICE_TAG, "Inserted body parts data.")
            } else {
                Log.d(STORAGE_SERVICE_TAG, "Body parts data already exists.")
            }
