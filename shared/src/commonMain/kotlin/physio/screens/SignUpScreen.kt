...
fun SignUpScreen(signupViewModel: SignUpViewModel) {
    ...
    val showProgress = signupViewModel.showProgress.collectAsState()

    LaunchedEffect(signupViewModel.signupMessage) {
        signupViewModel.signupMessage.collect { message ->
            if (message != null) {
                Toast.makeText(context, message, Toast.LENGTH_LONG).show()
                passwordState.value = TextFieldValue("")
                signupViewModel.clearSignupMessage()
            }
        }
    }

    fun performSignUp() {
        signupViewModel.email = emailState.value.text
        signupViewModel.password = passwordState.value.text
        signupViewModel.repeatedPassword = repeatedPasswordState.value.text
        signupViewModel.name = nameState.value.text
        signupViewModel.lastname = lastnameState.value.text
        signupViewModel.licenseNumber = licenseNumberState.value.text
        signupViewModel.performSignUp(context)
    }
        ...
        Text(
            ...
            text = buildAnnotatedString {
                ...
                val styleHighlight = SpanStyle(
                    color = colorPrimary,
                    fontFamily = FontFamily(Font(com.physio.android.R.font.helvetica_neue_medium))
                )
                append("Utwórz nowe ")
